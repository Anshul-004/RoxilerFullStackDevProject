...
export const Card = ({shopData}) => {
  ...
  const handleAddRating = async () => {
    ...
    try {
      ...
      const response = await axios.post('/api/rating/addRating', {
        rating: rating,
        shopId: shopData.id
      })
        ...
        <div className='space-y-2'>
          {!showRatingInput ? (
            <button 
              ...
              className='w-full border-2 border-amber-600 bg-amber-600 text-white rounded-xl py-2 px-4 font-medium hover:bg-amber-700 hover:border-amber-700 transition-colors duration-200'
            >
              Add Rating
            </button>
          ) : (
